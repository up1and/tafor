...
如果程序收到观测软件发来的 METAR 或 SPECI 报文，校验功能会被启用，如果没有收到观测报文，预览界面不会对报文进行逻辑校验。
...
画布模式会每 5 分钟加载一次最新的云图，并且在画布的左下角显示云图的更新时间，如果无法获取云图，底图会被灰色画布代替；区域编辑方式主要有四种，点，线，经纬度，走廊区域，点击区域右上角右一的 **淡蓝色图标** 可在四种状态之间切换。
